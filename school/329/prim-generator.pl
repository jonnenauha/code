...
for( my $j=1; $j < $euler_phi; $j ++ )
{
	$root = calc_exp_mod( $generator, $j, $modulus );
	...
	if( $is_prim_root )
	{
		print	"$generator^$j is a primitive root mod $modulus, where phi($modulus)=$euler_phi.\n";
	}
	else
	{
		print	"$generator^$j is NOT a primitive root mod $modulus, where phi($modulus)=$euler_phi.\n";
	}
